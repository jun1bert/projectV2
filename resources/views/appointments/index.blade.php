...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
    ...
    <div class="flex flex-col sm:flex-row gap-2 flex-1">
        ...
        <div class="relative flex-1 min-w-0 sm:max-w-xs">
            ...
            <input type="text"
                   ...
                   class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 transition">
        </div>

        <select id="statusFilter"
                class="w-full sm:w-40 px-3 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition">
    ...
    <button type="button" onclick="openWalkInModal()"
            class="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-sm font-medium rounded-xl transition shrink-0">
...
<div class="hidden lg:block bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
    ...
    <table class="w-full text-sm">
        ...
        <tr class="filter-row hover:bg-gray-50/70 transition-colors"
            ...
            <td class="px-5 py-4" data-price="{{ $a->service->price ?? 0 }}">
                ...
                <span class="block text-xs text-gray-400 mt-0.5">₱{{ number_format($a->service->price, 2) }}</span>
                @endif
            </td>
            ...
            {{-- STATUS --}}
            <td class="px-5 py-4">
                <span class="@statusBadge($a->status) inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
            ...
            <td class="px-5 py-4">
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <select class="status-select px-3 py-2 rounded-lg text-xs border border-gray-200 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition"
                            data-id="{{ $a->id }}">
                    ...
                    @if($canAssign)
                    <select class="assign-select px-3 py-2 rounded-lg text-xs border border-gray-200 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 transition"
                            data-id="{{ $a->id }}">
                    ...
                    <button type="button"
                            class="pay-btn inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white transition"
                            data-id="{{ $a->id }}">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <a href="{{ url('/invoices') }}/{{ $a->invoice->id ?? '' }}/receipt"
                       class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 transition">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<div class="filter-row bg-white border border-gray-100 rounded-2xl p-4 shadow-sm"
    ...
    <div class="border-t border-gray-100 pt-3 flex flex-wrap gap-2">
        ...
        <select class="status-select flex-1 min-w-[120px] px-3 py-2 rounded-xl text-xs border border-gray-200 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                data-id="{{ $a->id }}">
            <option value="pending"   @selected($a->status=='pending')>Pending</option>
            <option value="confirmed" @selected($a->status=='confirmed')>Confirmed</option>
            <option value="rejected"  @selected($a->status=='rejected')>Rejected</option>
        ...
        @if($canAssign)
        <select class="assign-select flex-1 min-w-[120px] px-3 py-2 rounded-xl text-xs border border-gray-200 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                data-id="{{ $a->id }}">
            <option value="">Unassigned</option>
            @foreach($staff ?? [] as $member)
                <option value="{{ $member->id }}" @selected($a->assigned_to == $member->id)>{{ $member->name }}</option>
        ...
        @if($a->status === 'completed' && ($a->payment_status ?? null) !== 'paid')
        <button type="button"
                class="pay-btn inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white transition"
                data-id="{{ $a->id }}">
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
            </svg>
        ...
        @elseif($a->status === 'completed' && ($a->payment_status ?? null) === 'paid')
        <a href="{{ url('/invoices') }}/{{ $a->invoice->id ?? '' }}/receipt"
           class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 transition">
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            Receipt
    ...
    @csrf
                ...
                <div>
                    ...
                    <input type="text" name="full_name" required
                           class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 transition"
                ...
                <div>
                    ...
                    <select name="service_id" required
                            class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 transition bg-white">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="date" name="date" required
                               class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 transition">
                    </div>
                    <div>
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <button type="submit"
                            class="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium transition">
                        Create appointment
                    </button>
                </div>
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div class="px-6 py-5 max-h-[80vh] overflow-y-auto">
                <form id="paymentForm" class="space-y-4">
                    ...
                    {{-- Summary Card --}}
                    <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 text-sm space-y-2">
                        ...
                        <div class="border-t border-gray-200 pt-2 flex justify-between">
                            ...
                            <span id="pm_total" class="font-bold text-indigo-600 text-base">₱0.00</span>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button type="button" id="addItemBtn"
                                    class="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700 font-medium transition">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                </svg>
                                Add item
                    ...
                    <div>
                        ...
                        <select name="method" id="payment_method" required
                                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-300 transition">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2"
                                  class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 transition resize-none"
                    ...
                    <div class="flex gap-2 pt-1">
                        ...
                        <button type="submit" id="paymentSubmitBtn"
                                class="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-sm font-medium transition flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                            </svg>
                            Confirm payment
...
let currentPayBtn = null;  // Reference to the button that opened the modal
...
function createItemRow() {
    ...
    row.innerHTML = `
        <div class="relative flex-1">
            <input type="text"
                   class="item-search w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-indigo-300 transition"
        ...
        <input type="number"
               class="item-qty w-16 px-2 py-2 border border-gray-200 rounded-xl text-xs text-center focus:outline-none focus:ring-2 focus:ring-indigo-300 transition"
...
document.addEventListener('input', async function (e) {
    ...
    data.forEach(item => {
        ...
        div.className = 'px-3 py-2.5 hover:bg-indigo-50 cursor-pointer text-xs transition border-b border-gray-50 last:border-0';
        div.innerHTML = `
...
paymentForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (data.success) {
    /* Open receipt in a new tab — appointments page stays open */
    ...
    if (currentPayBtn) {
        ...
        const receiptUrl = `/invoices/${data.invoice_id}/receipt`;
        const receiptLink = document.createElement('a');
        receiptLink.href = receiptUrl;
        receiptLink.target = '_blank';
        receiptLink.className = 'inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 transition';
        ...
        const statusCell = row.querySelector('[class*="bg-indigo-50"], [class*="bg-amber-50"], [class*="bg-green-50"], [class*="bg-red-50"]');
        if (statusCell) {
    ...
    /* Remove the old redirect line — no longer needed */
...
document.addEventListener('change', async function (e) {
    ...
    try {
        ...
        alert('Something went wrong while updating the assignment.');
    }
});

/* Track previous select value to allow revert on validation failure */
...
document.getElementById('walkInForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            closeWalkIn();
            location.reload(); // reload to show the new appointment in the list
        } else {
